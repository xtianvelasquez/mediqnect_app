...
def store_prescription(db: Session, medicine_data, medicine_compartment_data, intake_data, user_id):
  try:
    ...
    medicine_table = Medicine(**medicine_data)
    db.add(medicine_table)
    db.flush()  # get medicine_id
    db.refresh(medicine_table)

    # Insert into intake table
    intake_table = Intake(**intake_data)
    db.add(intake_table)
    db.flush()  # get intake_id
    db.refresh(intake_table)

    # Insert into prescription table
    prescription_table = Prescription(medicine_id=medicine_table.medicine_id, intake_id=intake_table.intake_id, user_id=user_id)
    db.add(prescription_table)

    medicine_compartment_table = Medicine_Compartment(compartment_id=medicine_compartment_data['compartment_id'], medicine_id=medicine_table.medicine_id, user_id=user_id)
